...
function generatePost(post) {
  ...
  const returnPostDate = (date) => `${
          ['Jab', 'Feb', 'March', 'Apr', 'May', 'Jun', 'July', 'Aug', 'Sep',
           'Oct', 'Nov', 'Dec'][date.getMonth()]} ${date.getDate()}, ${
      date.getFullYear()}`;
  ...
  article.innerHTML = `
                    <div class="post__meta">
                        ...
                        ${
      post.meta.tags.map((tag) => `<span class="post__tag">${tag}</span>`)
          .join('')}
                        </div>
                        <p class="post__date">${
      returnPostDate(new Date(post.meta.date))}</p>
                    </div>
      ...
      post.user.name[0].firstName} ${post.user.name[1].lastName}</p>
                            <p class="post__author--role"><small>${
      post.user.jobTitle}</small></p>
                        </div>
                    </div>
                    <p class="post__body">
                        ${post.summary}
...
const loadPosts =
    () => {
      ...
      filteredPosts.slice(0, maxDisplayPosts)
      ...
      postContainer.appendChild(frag);
          ...
          function filterPosts() {
            const searchFilter =
                (post) => [post.summary, post.user.jobTitle, post.title,
          ...
          document.querySelector('.btn--view')
              .addEventListener('click', viewMorePosts)
